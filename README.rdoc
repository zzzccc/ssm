...
2. cd ssm 
3. bundle install
4. gem install fluentd --no-ri --no-rdoc
5. gem install fluent-plugin-mongo --no-ri --no-rdoc
6. gem install vmstat --no-ri --no-rdoc 
...
- in your nginx.conf
  log_format ssm '$server_name $server_port $remote_addr - $remote_user [$time_local] '
...
and ssm.log will be like follow:
  ...
  - localhost 80 127.0.0.1 - - [13/Mar/2014:18:57:47 +0900] "GET / HTTP/1.1" 200 393 "-" "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_1) AppleWebKit/537.73.11 (KHTML, like Gecko) Version/7.0.1 Safari/537.73.11"

11. fluentd -c ssm/fluentd.conf ( run fluentd )
